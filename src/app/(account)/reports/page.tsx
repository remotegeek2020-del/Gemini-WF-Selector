...
function exportLeadsCSV(leads: Lead[], personaName?: string) {
  const rows = [
    ['First Name', 'Last Name', 'Email', 'Phone', 'Company', 'Title', 'LinkedIn Profile', 'Persona', 'Status', 'Created'],
    ...leads.map((lead) => {
...
export default function SubAccountReportsPage() {
  ...
  useEffect(() => {
    if (accountId) fetchLeadsByPersona(selectedPersonaId)
  }, [selectedPersonaId, fetchLeadsByPersona, accountId])
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader><CardTitle>Leads by Persona</CardTitle></CardHeader>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>
              {selectedPersonaId
                ? `Leads — ${personaReports.find((p) => p.persona_id === selectedPersonaId)?.persona_name || 'Selected'}`
                : 'All Leads'}
            </CardTitle>
            <div className="flex items-center gap-3">
              ...
              {selectedPersonaId && (
                <button onClick={() => setSelectedPersonaId('')} className="text-xs text-gray-500 hover:text-gray-700 underline">
                  Clear filter
                </button>
              )}
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  ...
                  {leads.length === 0 && (
                    <tr>
                      <td colSpan={5} className="py-8 text-center text-gray-400 text-sm">No leads found</td>
                    </tr>
                  )}
                </tbody>
              </table>
