            ...
            <div>
              ...
              >
                {Array.from({ length: 24 }, (_, h) => {
                  ...
                  const lbl = `${h % 12 || 12}:00 ${ampm}`
                  return <option key={h} value={h}>{lbl} UTC</option>
                })}
              </select>
            </div>
          </div>
...
function ScheduleList({ accountId, schedules, onChanged }: ScheduleListProps) {
  ...
  if (schedules.length === 0) {
    return <p className="text-sm text-gray-400 py-4 text-center">No schedules yet.</p>
  }
      ...
      {schedules.map((s) => (
        <div key={s.id} className="flex items-start justify-between py-3 gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button onClick={() => del(s.id)} className="text-gray-300 hover:text-red-500 transition-colors">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
