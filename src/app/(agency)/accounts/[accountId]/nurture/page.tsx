...
export default function NurtureDashboardPage({ params }: { params: { accountId: string } }) {
  ...
  const fetchLeads = useCallback(async () => {
    ...
    setError(null)
    try {
      const query = new URLSearchParams({ limit: '100', pipeline: 'nurture' })
      if (statusFilter) query.set('status', statusFilter)

      const res = await fetch(`/api/accounts/${accountId}/leads?${query}`)
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Nurture Leads</h1>
          <p className="text-sm text-gray-500 mt-1">
            Long-term nurture pipeline — {total} lead{total !== 1 ? 's' : ''}
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Nurture Leads</CardTitle>
        </CardHeader>
