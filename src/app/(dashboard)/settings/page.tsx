...
export default function SettingsPage() {
  ...
  const [formValues, setFormValues] = useState<Record<string, { key_value: string; extra_data: Record<string, string> }>>({})
  ...
  const getExistingKey = (service: string): ApiKeyEntry | undefined => {
    return existingKeys.find((k) => k.service === service)
  }

  const getFormValue = (service: string) => {
    return formValues[service] || { key_value: '', extra_data: {} }
  }
    ...
    <div className="max-w-2xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Configure API keys for external services. Keys are stored securely in your database.
        </p>
      </div>
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <p className="text-sm text-gray-600 mb-2">
            Configure this URL in your Highlevel account to receive lead webhooks:
          </p>
          <div className="flex items-center gap-2">
            <code className="flex-1 bg-gray-100 px-3 py-2 rounded text-xs text-gray-800 font-mono break-all">
              {typeof window !== 'undefined' ? window.location.origin : 'https://your-app.com'}/api/webhook/highlevel?secret=YOUR_WEBHOOK_SECRET
            </code>
          </div>
          <p className="text-xs text-gray-500 mt-2">
            Set the WEBHOOK_SECRET environment variable in your deployment to secure this endpoint.
          </p>
        </CardContent>
      </Card>
